...
## v10 (2026-08-12 14:00~, 공식 챌린지 코스 50범프 — 진행 중)

> 발표용 요약 섹션. 순서대로 읽으면 "무엇이 문제였고 → 무엇을 바꿨고 → 점수가 어떻게 움직였나"가 된다.

### 1. 문제 정의 (한 장으로)

- 공식 코스: 장애물 **50개**, 목표 `GOAL_X=101.9 m`
- 채점 = `evaluate.py`가 **20초(1000스텝) 동안 도달한 최대 x**. 결정적(`seed=0`, `reset_noise=0`, greedy) 1회 주행
- 완주는 평균 5.1 m/s가 필요 → 사실상 불가. 따라서 **완주 문제가 아니라 "20초 거리 최대화" 문제**
- 발표 포인트: 목표 함수가 "생존"이 아니라 **"평균 속도 × 20초"** 라는 재정의가 v10의 출발점

### 2. 출발점과 병목 (v9 인수 시점)

| 항목 | 값 |
|---|---|
| 체크포인트 | `bump_challenge_v9_8bumps_final/walker_model_2600000_steps.zip` (구 8범프 맵 2.6M) |
| 공식 점수 | **31.77 m** |
| 평균 속도 | 1.59 m/s (낙상 없음, 1000/1000 스텝) |
| 병목 | x≈26.4 (bump7·8 = 0.5→1.0m 계단)에서 **8.6초 정체** |

`diag_run.py`로 x/z/속도를 50스텝마다 찍어 병목 위치를 특정한 것이 이후 모든 판단의 근거가 됐다.

### 3. Phase A — 보상 무변경 파인튜닝 (코스 자체가 커리큘럼)

공식 코스는 앞이 쉽고 뒤가 어려우므로, 보상을 건드리지 않고 그냥 이어 학습했다.

| 스텝 | 점수 | 평균 속도 |
|---|---|---|
| 2.6M (v9) | 31.77 m | 1.59 m/s |
| **3.0M** | **45.90 m** | 2.30 m/s |
| 3.2M | 45.87 m | 2.29 m/s |
| 3.4M | 45.83 m | 2.29 m/s |

- **400k 스텝에 +14.1 m.** 8.6초 정체가 사실상 해소됨 (지형 일반화 효과)
- 그 뒤 400k는 45.9에서 완전 정체 → **보상 구조가 2.3 m/s에서 균형점을 찾았다**는 신호. 보상을 바꿀 근거 확보

### 4. 처리량 문제 (엔지니어링 교훈, 발표에 넣을 가치 있음)

3시간이라는 시간 예산에서 **처리량 자체가 성능 상한**이었다. 두 가지를 고쳤다.

1. `learning.py`가 `--bump_challenge`에서 `DummyVecEnv`(단일 프로세스)를 강제하고 있었다 → `SubprocVecEnv`로 전환.
   200k 스텝당 **11분 → 4.5분** (약 660 → 1300 steps/s, 12코어 사용). 채점에는 무관하고 학습 속도만 오름
2. 보상 A/B를 두 런 병렬로 돌리자 코어 경쟁으로 **둘 다 절반 속도**가 됐다 → 병렬 폐기, 한 런에 전 코어.
   대신 **순차 A/B**(최고 체크포인트에서 보상만 바꿔 재개)로 비교

부수적으로 다른 세션이 남긴 유령 학습 프로세스(`--bump_practice`)가 CPU를 먹고 있던 것도 정리했다.
→ 교훈: **학습 시작 전 `Get-CimInstance Win32_Process`로 경쟁 프로세스와 VecEnv 종류를 먼저 확인할 것.**

### 5. Phase B-1 — "속도 보상" 패키지 (3.4M에서 재개)

점수가 거리이므로, 보상에서 "버티기"의 값을 깎고 "전진"의 값을 올렸다.

| 변경 | 이전 | 이후 | 이유 |
|---|---|---|---|
| 전진 가중치 | `60 * dx` | `100 * dx` | 3 m/s에서 6.0/step. 속도 자체의 이득 확대 |
| healthy 보상 | `1.0` 상수 | 1초(50스텝) 이상 최고 x 갱신 없으면 `0` | 정체 중 +1/step 수익 제거 (2.3 m/s에서 "1초 서 있기" = 전진 0.36 m와 등가였다) |
| stall 종료 | 500스텝(10초) | 250스텝(5초) | 정체 에피소드를 빨리 끊어 샘플 효율↑ (채점기는 우리 종료를 무시하므로 점수 영향 없음) |

| 스텝 | 점수 | 평균 속도 | 비고 |
|---|---|---|---|
| 3.6M | 44.45 m | 2.22 m/s | 보상 변경 직후 일시 하락 |
| 3.8M | **48.53 m** | 2.43 m/s | 신기록 |
| 4.0M | 14.64 m | 3.15 m/s | **더 빨라졌지만 step 232 낙상** |
| **4.2M** | **48.58 m** | 2.43 m/s | 최고 |
| 4.4M | 28.04 m | 2.58 m/s | 낙상 |

- **31.77 → 48.58 m (+53%)**
- 관찰: 속도를 올리면 낙상 체크포인트가 섞여 나온다. 채점이 결정적이므로 **200k마다 전수 채점해서 최고본만 고르면 되는 구조**
  (그래서 `watch_eval.ps1`로 새 체크포인트 자동 채점 → `scores.txt` 기록을 자동화했다)

### 6. 진단: 남은 손실은 전부 "장애물 감속" (핵심 슬라이드)

4.2M(48.58 m) 트레이스 (`diag_run.py --every 50`):

```
평지·잔범프 구간:  3.5 ~ 4.05 m/s   ← 보행 속도는 이미 충분
큰 범프 통과 시:   0.4 ~ 1.0 m/s    ← 매번 약 1~2초 손실
```

| 구간 | 지점 | 손실 |
|---|---|---|
| step 430~540 | bump7·8 (0.5→1.0m 계단) | ~2.2초 |
| step 680~740 | bump12 (0.35m) | ~1.2초 |
| step 860~940 | bump18 (0.9m) | ~1.5초 |
| 합계 | | **약 4.5초 ≈ 14 m** |

- v9의 8.6초 단일 병목 → **여러 장애물의 1초짜리 감속으로 분산**된 상태. 성격이 완전히 바뀌었다
- bump2(0.5m 벽)는 이제 **3.53 m/s로 감속 없이 통과**한다 (v9에서는 1초 손실)
- 종료 사유는 항상 `시간 초과(정상 종료)`, 1000/1000 스텝 → **넘어져서 멈춘 게 아니라 20초가 끝난 것**
- 1등 팀 83 m = 평균 4.15 m/s. 우리 평지 최고가 4.05 m/s이므로 **격차는 보행 능력이 아니라 감속 손실 + 출발 가속(첫 1초 ~1 m/s)**

### 7. Phase B-2 — "빠르게 통과" 보상 (4.2M에서 재개, 진행 중)

6절의 진단에 맞춰, 보상이 **"높이 뛰기"를 칭찬하고 "빠르게 넘기"는 칭찬하지 않던 것**을 고쳤다.
(bump8에서 torso z=1.70~2.03까지 솟구쳤는데, 체공 시간은 전진에 거의 기여하지 않는다.)

| 변경 | 이전 | 이후 |
|---|---|---|
| 큰 범프 통과 보너스 | `+50` 고정 | `30 + 30 * clip(vx/3.5, 0, 1)` — 기어 넘으면 30, 3.5 m/s로 넘으면 60 |
| 등반 높이 보너스 | `+15` | `+10` (순수 수직 도약 유인 축소) |

- 설계 원칙 준수: **1회성 플래그 보너스만** 추가/수정. 매 스텝 양수 shaping은 추가하지 않았다 (교훈 1·5)
- 진동으로 착취 불가 (`passed_bumps[i]` 단조 플래그 + 범프 뒷면 기준)

### 7-1. Phase B-3 — 변동성 확대 + 언덕오르기 (결정적 채점의 극단적 활용)

채점이 결정적 1회라는 성질을 끝까지 썼다: **평균 성능은 무의미, 역대 최고 체크포인트 1개만 중요.**

- 변동성 확대: `ent_coef` 0.01→0.03, `target_kl` 0.03→0.05, `log_std` 하한 -0.5, 저장 주기 200k→**100k** (복권 추첨 횟수 2배)
- 효과 즉시: 재시작 후 첫 100k 체크포인트가 **53.07 m** (48.58 → +9%)
- `hillclimb.ps1` 자동화: 항상 최고점에서 학습 재시작, 신기록이면 그 지점으로 이동, 6회 무개선이면 재추첨
  (SB3 zip이 RNG를 저장하지 않아 재시작마다 탐색 경로가 달라짐 = 같은 기점에서 매번 다른 시도)

### 7-2. problem3.md 이식 (팀 병렬 실험, 58.1 m 검증)

팀의 병렬 실험(problem3.md, Run A/B 전략)에서 58.1 m이 검증되어 4개 항목을 이식:

| 항목 | 변경 | 의미 |
|---|---|---|
| `MIN_TORSO_Z` | 0.9 → 0.7 | 학습 종료 기준(0.9)과 평가 낙상 기준(0.5)의 **OOD 갭 해소**. 고속 보행의 상체 진동 영역을 학습이 경험 → 낙상 체크포인트 감소 기대 |
| `speed_gate` | 1→3 m/s 포화 | 대칭 보행 보너스가 1 m/s에서 포화 → "빠르면서 대칭"에 인센티브 부여 |
| 안정화 페널티 | 속도 연동 (3 m/s에서 80% 감면) | 고속의 필연적 진동을 벌점 주면 속도 상한이 걸림 |
| 속도 목표 보상 | 2.5 m/s 초과분 ×0.5 (상한 +2.0), 범프 근처 80% 감쇠 | 매 스텝 양수 shaping이지만 팀 실험에서 검증됨 |

이식 직후 100k~300k는 재적응 하락(28~46 m) 후 회복 추세(4.8M 47.7 → 4.9M 52.86).

### 7-3. v10 최종 (Phase C)

- **최종 제출본: `checkpoints/bump_challenge_v10_final/walker_model_4300000_steps.zip` = 53.07 m**
  (평균 2.65 m/s, 낙상 없음, 1000/1000 스텝. 제출 리포지토리 코드로 재검증 완료 — 결정적 동일 점수 확인)
- v9 대비 **31.77 → 53.07 m (+67%)**
- 주의: 이 체크포인트는 practice2_speed의 보상으로 학습됐지만, **관측 코드·`OBS_NORM_*`은 본체와 동일**하므로
  본체 `custom_walker2d.py`로 채점해도 점수가 같다 (보상/종료는 greedy 행동 수집에 영향 없음)

### 7-4. 도박 실험 — 점프 강도↑ + 다리벌림(런지) 보너스 (기각)

53.07m 최고본에서 "더 극단적인 방법"으로 두 가지를 시도:

| 변경 | 내용 |
|---|---|
| 도약 임펄스/높이 보너스 | +10 → **+20**, 임계값도 완화 (더 쉽게, 더 세게 터지도록) |
| 다리 벌림(런지) 보너스 (신규) | 큰 범프 2m 전부터 좌우 허벅지 각도차에 비례해 **매 스텝** `0.8×min(각도차,1.5)` 지급 |

결과: 4.4M 35.20m(낙상), 4.5M 11.88m(**뒤로 넘어짐**, 5.4초), 4.6M 48.13m(생존). 전부 53.07 미달.
**원인 추정**: 다리벌림 보너스가 매 스텝 양수 shaping이라(원칙 위배) 도약 직전 과도한 런지 자세를 만들었고,
점프 강도 상승과 겹쳐 착지 시 무게중심이 뒤로 쏠리는 낙상 패턴을 유발한 것으로 보임.
→ **기각, 53.07m 최종본 유지.** (교훈 6 "수직 도약 ≠ 장애물 통과"의 반례를 하나 더 확인한 셈)

### 8. v10 교훈 (발표 마무리용)

1. **목표 함수를 먼저 정확히 읽어라.** 점수가 "20초 거리"인데 보상은 "생존 +1/step"을 주고 있었다. healthy 상수 보상은 정체 8.6초를 흑자로 만들어 준다
2. **진단 도구가 보상 설계보다 먼저다.** `diag_run.py`의 x/z/속도 트레이스로 "어디서 몇 초를 잃는지"를 수치화하지 않으면, 어떤 보상을 만질지 고를 수 없다
3. **코스가 곧 커리큘럼일 수 있다.** 별도 커리큘럼 XML 없이 공식 코스 파인튜닝만으로 +14 m를 얻었다
4. **처리량은 성능이다.** VecEnv 종류 하나로 학습 속도가 2배 차이났다. 시간 예산이 정해진 과제에서는 이게 곧 점수
5. **결정적 채점은 "최고본 고르기" 전략을 허용한다.** 낙상 체크포인트가 섞여도 문제없다. 전수 채점 자동화가 이득. 변동성(entropy)을 키우고 저장 주기를 촘촘히 하면 "복권 추첨 횟수"를 늘릴 수 있다 (48.58 → 53.07, 100k 스텝 만에)
6. **수직 도약 ≠ 장애물 통과.** 높이 보너스는 등반을 가능하게 하지만, 과하면 "제자리 높이뛰기"로 수렴한다. 통과 보너스를 통과 속도에 연동해야 한다. 이 교훈을 무시하고 점프 강도와 매 스텝 런지 보너스를 더 올려본 도박(7-4)은 낙상만 늘리고 기각됐다 — **매 스텝 양수 shaping 금지 원칙이 실전에서도 다시 확인됨**
7. **모든 실험은 "최고점에서 분기 + 실패 시 즉시 롤백"으로.** 되돌리기가 공짜인 결정적 채점 구조에서는, 나쁜 방향은 즉시 버리고 항상 검증된 최고 체크포인트로 돌아가는 것이 손해 없는 전략이다

### 9. v10 도구 추가

| 파일 | 용도 |
|---|---|
| `practice2_speed/` | 보상 A/B용 격리 사본 (`practice2/`의 코드는 보존). 검증 후 승자만 본체에 이식 |
| `practice2_speed/watch_eval.ps1` | 새 체크포인트를 자동 채점해 `scores.txt`에 누적 (전수 채점 자동화) |
| `checkpoints/v10_best/` | 각 단계의 최고 점수 체크포인트 백업 (제출 안전판) |
...
| 2.6 | 31.77 | v9 인수 |
| 3.0 | 45.90 | Phase A |
| 3.4 | 45.83 | Phase A (포화) |
| 3.8 | 48.53 | B-1 (속도 보상) |
| 4.2 | 48.58 | B-1 |
| **4.3** | **53.07** | **B-3 (변동성 확대) — 최종 제출본** |
| 4.9 | 52.86 | problem3 이식 후 회복 중 |
