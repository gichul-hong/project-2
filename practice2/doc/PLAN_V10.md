...
## 8. 진행 체크리스트 (완료, 2026-08-12 16:52 마감)

- [x] Phase A: 공식 코스 파인튜닝, 2.6M→3.4M (점수: 31.77 → 45.90 → 45.83, 포화)
- [x] Phase B-1: healthy 보상 전진 게이트 + 전진 가중치 60→100 + stall 500→250 (점수: 48.53~48.58)
- [x] Phase B-3: 변동성 확대(ent_coef/target_kl/log_std↑) + 언덕오르기 자동화 (점수: **53.07, 최종**)
- [x] problem3.md 이식 (MIN_TORSO_Z 0.7 등, 팀 58.1m 검증분) — 재적응 중 최고 52.86, 53.07 못 넘음
- [x] 도박 실험(점프 강도↑ + 다리벌림 매스텝 보너스) — 낙상 다발(11.88~48.13), **기각**
- [x] Phase C: 최종 체크포인트 선정 + 기록 (영상 생략, 시간 예산 종료)
- 상세 이력은 `TRAINING_HISTORY.md` v10 섹션(§1~10) 참고
