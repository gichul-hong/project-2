            ...
            for i, b in enumerate(self.bumps):
                ...
                if b["big"] and not self.passed_bumps[i] \
                    ...
                    if not self.jump_bonus[i] and \
                        ...
                        reward += 20.0  # 도박: 10->20, 임계값도 낮춤(더 자주 터짐) -> 도약 자체를 강하게 유인
                        self.jump_bonus[i] = True
                    if not self.height_bonus[i] and torso_z > b["height"] + 1.05:
                        reward += 20.0  # 도박: 10->20
                        self.height_bonus[i] = True
                    # 도박: 도약 직전 "다리 벌림"(스트라이드) 유인. 큰 범프 2m 전부터 다리
                    # 사이 각도차가 클수록(런지 자세) 보상 -> 도약 파워를 위한 준비 동작 형성.
                    # thigh 각도차(obs[3]-obs[6])가 클수록 다리가 벌어진 상태.
                    leg_split = abs(float(obs[3]) - float(obs[6]))
                    reward += 0.8 * min(leg_split, 1.5)
